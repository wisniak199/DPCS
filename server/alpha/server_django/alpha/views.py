...
def add_crash_report(request):
    if(request.method=='POST'):
        ...
        new_crash_report = CrashReport(crash_report_id=cr_id, crash_group_id=cg,
...
def add_solution(request):
    if(request.method=='POST'):
        ...
        new_sol = Solution(solution_id=sol_id, crash_group_id=cg,
...
def crash_report_list(request):
    ...
    if request.method == 'GET':
        ...
        if added == -1:
            return Response("Could not add the crash report ..."
...
def crash_group_add(request):
    ...
    if(request.method=='GET'):
        return render_to_response('alpha/crash_group_add.html',
...
def crash_report_add(request):
    reports = CrashReport.objects.all()
    if(request.method == 'GET'):
        return render_to_response('alpha/crash_report_add.html',
...
def solution_list(request):
    if request.method == 'POST':
        ...
        if new_sol_id == -1:
            return Response("Could not add the solution ..."
                "Make sure you passed correct arguments ...",
                status=status.HTTP_400_BAD_REQUEST)
